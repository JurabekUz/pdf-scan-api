...
class DocumentController extends AbstractDocumentController {
    ...
    async deleteDocument(req: Request, res: Response): Promise<void> {
        try {
            await DocumentSchema.findByIdAndUpdate(req.params.id, {is_delete: true});
            res.status(200).json({
                ok: true,
    ...
    async updateDocument(req: Request, res: Response): Promise<void> {
        try {
            ...
            const updatedDocument = await DocumentSchema.findByIdAndUpdate(req.params.id, newDocument, {new: true});
            res.status(200).json({
                ok: true,
